...
ebos %>% 
  group_by(sexo) %>% 
  summarise(numero_de_amostras = n()) %>% 
  mutate(proporcao_de_amostras = 
           scales::percent(numero_de_amostras/sum(numero_de_amostras))) %>% 
...
```{r, fig.align='center', fig.height=8, fig.width=7, echo=FALSE, max.print = 1}
...
plot <- vertebras_prop %>% 
  ...
  geom_point() +
  geom_line(group = 1) +
  geom_label(aes(label = n_ebos), vjust = -.4, hjust = 1.2) +
  geom_label(aes(label = scales::percent(prop)), vjust = -.4, hjust = -.01) +
